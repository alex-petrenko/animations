...
number_font = "Latin Modern Mono Light"
...
def texy_text(text, **kwargs):
    ...
    text = Text(text, font="Latin Modern Mono Light", weight="light", stroke_width=0, **kwargs)
    # text = Text(text, font='Open Sans', weight='LIGHT', **kwargs)
    # text = Text(text, font='Latin Modern Mono', weight='thin', **kwargs)
    return text
    ...
    def construct(self):
        ...
        cpu_util_text, cpu_util_number = cpu_util_label = VGroup(
            ...
            DecimalNumber(
                ...
                include_sign=False,
                font_size=24,
                text_config={"font": number_font, "color": BLUE_B},
            ),
        )
        ...
        gpu_util_text, gpu_util_number = gpu_util_label = VGroup(
            ...
            DecimalNumber(
                ...
                include_sign=False,
                font_size=24,
                text_config={"font": number_font, "color": ORANGE},
            ),
        )
        ...
        x_axis = NumberLine(include_tip=True, include_ticks=False, width=12)
        ...
        worker_list.arrange_in_grid(n_rows=5, n_cols=1, buff=0.5).to_edge(LEFT, SMALL_BUFF)
        ...
        self.play(
            ShowCreation(x_axis),
            ShowCreation(x_axis_label),
            ShowCreation(worker_list),
            ShowCreation(total_time_label),
            ShowCreation(stats),
            ShowCreation(header),
        )
        self.wait(2)
        ...
        f_always(total_time_number.set_value, lambda: self.time - initial_time)

        def timestep_animation(rect, target_w):
            curr_w = rect.get_width()
        ...
        for iteration in range(num_iterations):
            for step_idx, step_times_og in enumerate(step_times_arr):
                ...
                env_step_rects = [
                    Rectangle(width=width, height=0.3, stroke_width=stroke, fill_color=BLUE_B, fill_opacity=1)
                    for i in range(num_workers)
                ]
                for i, t in enumerate(env_step_rects):
                    t.move_to(worker_texts[i].get_center(), LEFT, coor_mask=np.array([0, 1, 0]))
                    t.move_to(align_with + ofs, LEFT, coor_mask=np.array([1, 0, 0]))
                    self.add(t)
                ...
                inference_rect.move_to(env_step_rects[0].get_top(), TOP, coor_mask=np.array([0, 1, 0]))
                ...
                if step_idx == len(step_times_arr) - 1:
                    ...
                    backprop_rect.move_to(env_step_rects[0].get_top(), TOP, coor_mask=np.array([0, 1, 0]))
        ...
        self.play(WiggleOutThenIn(util_labels))
        self.play(WiggleOutThenIn(total_time_label))
...
class AsyncRL(Scene):
    def construct(self):
        ...
        # create a manim x axis
        x_axis = NumberLine(include_tip=True, include_ticks=False, width=11.5)
        ...
        policy_worker = texy_text("Policy worker", font_size=22)
        learner = texy_text("Learner", font_size=22)

        worker_list = VGroup(*(rollout_worker_texts + [policy_worker, learner]))
        worker_list.arrange_in_grid(n_rows=num_workers + 2, n_cols=1, buff=0.5).to_edge(LEFT, SMALL_BUFF)
        ...
        # TODO
        # self.play(ShowCreation(x_axis), ShowCreation(x_axis_label), ShowCreation(worker_list), ShowCreation(stats), ShowCreation(header))
        # self.wait(2)
        ...
        while True:
            ...
            for rwi, rws in enumerate(rollout_worker_states):
                ...
                if rws.rect is None and rws.has_actions and rws.work:
                    ...
                    rollout_rect = Rectangle(
                        width=width, height=0.3, stroke_width=stroke, fill_color=BLUE_B, fill_opacity=1
                    )
            ...
            if (
                rw_awaiting_action
                and len(rw_awaiting_action) >= min(min_action_requests_in_queue, active_rw)
                and inference_worker_state.rect is None
            ):
                inference_rect = Rectangle(
                    width=width, height=0.3, stroke_width=stroke, fill_color=ORANGE, fill_opacity=1
                )
            ...
            if learner_state.rect is None and learner_state.num_trajectories >= num_workers:
                learner_rect = Rectangle(
                    width=width, height=0.3, stroke_width=stroke, fill_color="#ffaf75", fill_opacity=1
                )
...
class DoubleBuffered(Scene):
    def construct(self):
        ...
        # create three squares demonstrating that blue and orange colors correspond to simulation, inference and backpropagation
        simulation_square_1 = Square(side_length=0.3, color=BLUE, stroke_width=0)
        simulation_square_1.set_fill(BLUE, 1)
        ...
        while True:
            ...
            for rwi, rws in enumerate(rollout_worker_states):
                ...
                if rws.rect is None and rws.has_actions[rws.split_idx()] and rws.work:
                    # ready to process next step
                    color = BLUE if rws.split_idx() == 0 else BLUE_B

                    rollout_rect = Rectangle(
                        width=width, height=0.3, stroke_width=stroke, fill_color=color, fill_opacity=1
                    )
                    rollout_rect.move_to(rollout_worker_texts[rwi].get_center(), LEFT, coor_mask=np.array([0, 1, 0]))
                    rollout_rect.move_to(orig_x_coord + total_time + ofs, LEFT, coor_mask=np.array([1, 0, 0]))
                    rollout_rect.c_type__ = "cpu"
